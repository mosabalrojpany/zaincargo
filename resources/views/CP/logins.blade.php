...
@section('content')
    ...
    <div class="d-flex justify-content-between">
        <h4 class="font-weight-bold">حركات الدخول {{ number_format($logins->total()) }}</h4> 
    </div>
    <!--    End header    -->
            ...
            <form class="row justify-content-between" action="{{ Request::url() }}"  method="get">
                ...
                <div class="form-inline col-auto">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control mx-sm-2 setValue" value="{{ Request::get('user') }}" name='user' required>
            ...
            <table class="table table-striped text-center">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><a href="{{ url('/cp/users#'.$login->user->id) }}">{{ $login->user->name}}</a></td>
                        <td>{{ $login->os}}</td>
                        <td>{{ $login->browser }}</td>
                        <td>{{ $login->country }}</td>
                        <td>{{ $login->city }}</td>
