...
@section('content')
    ...
    <p>
        الشحنة رقم <b>({{ $shipment->id}})</b> على العنوان <b><bdi>{{ $shipment->address->name }}</bdi></b> 
        من <b><bdi>{{ $shipment->address->country->name }}</bdi></b> وصلت , الرجاء قراءة وصف الحالة في المنظومة
    </p>
